...
| **Map Maker** | Add data by upload, from your library, or from online sources. The place search sits above **+ Add data** in the left panel (the 🔍 button under the on-map controls jumps to it). Choose from open vector and raster basemaps (or a blank canvas), or switch to any of 13 map projections with full pan/zoom/rotate. Symbolize with single symbol, unique values, graduated colour, or graduated point size. Filter, calculate new fields, browse the attribute table and field list, and run 18 spatial tools. |
...
**On-map controls.** Under the zoom buttons sit three tools: a **globe view**, a **measure-distance** tool (click points, read the running great-circle total, double-click to finish), and a **basemap-layers** panel that turns parts of the vector basemaps on and off — country/admin borders, place labels, roads and railways, water, buildings, land use and parks, and points of interest. The **globe view** wraps the actual basemap onto a true 3-D sphere (MapLibre GL's globe projection), keeping the basemap's own styling and overlaying your data layers on top — drag to spin, scroll to zoom, and click **Back to flat map** to return. It's a live rendered globe rather than the flat orthographic projection preview offered under Projections. While the globe is open you can still switch basemaps from the Basemap dropdown (the globe rebuilds to match), use **Fit data** to frame all your layers, **Whole globe** to return to the whole sphere sized to fill the panel (which is also how it opens), the compass beside the zoom buttons to return to north-up after rotating, a layer's 🔍 button to fly to just that one, and toggle basemap parts — the flat-map Projection dropdown is greyed out because it doesn't apply on the globe. Points draw as coloured circles on the globe (keeping each feature's colour), and polygons and lines keep their fills and outlines; custom point symbols and the dot-density/proportional renderers fall back to circles, so the flat map stays the reference for exact symbology. **Send to Composer** works from the globe too — it drops the current globe view onto the layout as an image (no scale bar, since scale isn't constant across a globe).
...
**Density mapping.** Two further modes for point layers. **Heatmap** draws a continuous density surface (Leaflet.heat) with adjustable radius and blur, optionally weighted by a numeric field — good for showing where things cluster, though it has no classes and so generates no legend entry. **Hexbin** aggregates the points into a hexagonal lattice and shades each cell, which gives you a density map you *can* read off a legend. Cell size is set in kilometres (the lattice is built in local kilometre coordinates, so cells stay a constant real size rather than stretching towards the poles), and each cell can report a point **count** or the **sum / mean / min / max** of a chosen field. **Create a layer from these bins** turns the cells into a proper polygon layer, putting the values in the attribute table where they can be filtered, classified and joined like anything else.
...
**Attribute table.** Opens as a floating window you can drag and resize, so the map stays live underneath. Dock it to the bottom or the right when you want map and table side by side, with a divider to set its size. It stays open as you switch layers, reloading for whichever layer is active, and remembers its size between sessions. Its toolbar is four buttons that each expand a panel below it, marked with a caret that turns down when open (only one panel shows at a time): **▤ Field options**, **✎ Edit attributes**, **Filter** and **Σ Statistics**. **Field options** lists the layer's fields with their types and how populated they are, and holds **＋ Add field** and the **ƒx Field calculator**. **Edit attributes** has three modes — *Off* (read-only), *Type into cells*, and *Paste a column* — only one active at a time, and editing switches off automatically when you close the panel or open another one. Pasted values land on the rows in the order shown. Edits are held as *pending* changes, highlighted in the table and counted in the panel, until **✓ Save edits** writes them to the dataset; **Clear all edits** discards them. Staging edits this way is what makes them stick: earlier versions wrote each cell straight into memory without saving the dataset, so edits could disappear when the table was closed and reopened. *Apply this filter to the table* sits inside the Filter panel. **Add field** asks only for a name: the column starts empty and takes its type from what you enter, so a column of numbers behaves as numeric everywhere. Edit values in place with the edit toggle, and open **Statistics** for a per-field summary (count, min, max, mean, median, sum, standard deviation): the statistics panel starts empty with a chooser of the layer's numeric fields, so on a wide dataset you tick only the fields you want rather than scrolling a huge table (with All / None shortcuts).
...
**Geocoding addresses** — the **Geocode addresses** section of the **Tools** tab takes a CSV with an address or place-name column and maps every row as a point, keeping the other columns as attributes (plus the matched name it found). It picks the likely address column for you, and you can add a place (e.g. *Kelowna, BC, Canada*) to narrow the search. It uses OpenStreetMap's free Nominatim service, so it deliberately runs one row per second and stops at 250 rows per go, with a progress bar and a Stop button — the panel says so before you start. Rows it can't match are skipped rather than mapped wrongly, and it reports how many. Results are approximate: check them on the map.

**Feature editing (digitising)** — the **Edit features** section of the **Tools** tab. Create a new empty **point**, **line** or **polygon** layer and draw straight into it, or select an existing layer and **Start editing** to reshape what's already there. The drawing toolbar appears at the top-right of the map (powered by [Leaflet-Geoman](https://geoman.io/)): draw points, lines, polygons and rectangles, drag whole features, move individual vertices, cut holes in polygons, rotate, and delete — with snapping to nearby vertices switched on. The map gets a gold outline while you're editing, and the panel shows **✓ Save edits** / **Cancel**, so nothing is written until you commit (Cancel warns before discarding). New features are created with the layer's existing fields present but empty, ready to fill in from the attribute table. If the layer's dataset is shared with other projects, Map-O-Matic offers to branch a private copy first, so digitising in one project can't quietly rewrite another's data. Whenever you save feature edits, Map-O-Matic asks whether you want to work on the layer's attributes, and if so opens the attribute table at the new rows with the Edit attributes panel ready. Editing works on the flat Web Mercator map — not in the globe view or a projected preview.
...
Elements: map frame (image, or from Map Maker), inset/locator map, title and text, legend, north arrow (3 styles), scale bar, neatline (plain or tick-marked), images/logos, and sketching. Selecting an element in the Layers tab opens its properties. The **inset/locator map** has the same live basemap engine as the main map — a basemap dropdown offering every basemap (vector, raster, satellite, or blank) and the same basemap-parts toggles (borders, place labels, roads, water, and so on) — plus optional data overlays and a locator box; it opens on whatever basemap Map Maker is currently showing so the two match by default.

All of Map-O-Matic's dialogs behave like real windows: drag one by its title bar to move it, and close it with the **✕** in the corner (the dimmed backdrop stays, so one dialog is in focus at a time).

**Right-click on the page** for a menu that saves the real pixels: *Save whole page as PNG*, or *Save selected element as PNG*, plus bring-to-front / send-to-back / delete for the selected element. Use this rather than the browser's own "Save image as…", which grabs the transparent interaction layer Fabric.js draws on top and writes a blank file.
